...
{
    uint32_t p, i;
    p = ((p_start+PGSIZE-1) & ~(PGSIZE-1));
    for(i = 0; p + PGSIZE <= p_end; p += PGSIZE, i++)
    {
        pmem[i].paddr = p;
        pmem[i].vpte = NULL;
...
void do_TLB_Helper()
{
    ...
    asm volatile("tlbp");  //invalid在填充TLB 项之前，使用TLBP指令来找出匹配但是无效的那一项的index值
    uint32_t index = get_cp0_index();
    uint32_t entrylo0, entrylo1;
    if(index & 0x80000000) {  //TLBP没有成功时P置 1
        //TLB refill
        set_cp0_entryhi(entryhi);
        entrylo0 = (current_running->pagetable[context >> 12]| PTE_C) ;
        entrylo1 = (current_running->pagetable[context >> 12 | 1] | PTE_C) ;
        ...
        if(!(current_running->pagetable[context >> 12] & PTE_V)){  //成功找到TLB索引值但无效
            if(emptylist.head != NULL){
                ...
                current_running->pagetable[context >> 12] = frame->paddr & 0xfffff000 | PTE_C | PTE_D | PTE_V;
                frame->vpte = &current_running->pagetable[context >> 12];
                frame->vaddr = context | current_running->pid<<2;
                frame->is_valid=1;
                frame = palloc(); //奇：分配一个可利用的物理页框
                current_running->pagetable[context >> 12 | 1] = frame->paddr & 0xfffff000 | PTE_C | PTE_D | PTE_V;
